...
export function SettingsPanel({ onConnected, onDisconnected }: SettingsPanelProps) {
  ...
  function handleChange(field: keyof ConnectionSettings, value: string) {
  ...
  function handleSave() {
    if (!form.apiToken) {
      setError(t.errorApiTokenRequired);
      return;
    }
    saveConnectionSettings(form);
    setForm(form);
    setIsConnected(true);
    onConnected();
  }
  ...
  function handleClear() {
    clearConnectionSettings();
    setForm({ apiBaseUrl: API_BASE_URL, apiToken: "" });
  ...
  return (
    <div className="p-4 space-y-4">
      ...
      <div className="space-y-1">
        <label className="text-xs font-medium text-gray-500">{t.edgeAdminApiUrl}</label>
      ...
      {/* API Token */}
      <div className="space-y-1">
        <label className="text-xs font-medium text-gray-500">{t.apiToken}</label>
      ...
      <div className="flex justify-between items-center pt-3 border-t border-gray-100">
        <button
          onClick={handleClear}
          className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-lg border border-[#E24B4A] bg-[#FCEBEB] text-[#791F1F] hover:bg-[#E24B4A] hover:text-white transition-colors"
        >
        ...
        <button
          onClick={handleSave}
          className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-lg bg-[#534AB7] text-white hover:bg-[#3C3489] transition-colors"
        >
          {t.saveAndConnect}
        </button>
      </div>
